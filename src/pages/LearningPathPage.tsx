...
export default function LearningPathPage() {
  ...
  useEffect(() => {
    if (placementTestData?.placementTestScore) {
      ...
      if (score >= 800) {
        // Điểm cao: current = cao cấp, target = 800+
        setCurrentLevel("lr_550_650_sw_200_250")
        setTargetLevel("lr_800_sw_300")
      } else if (score >= 550) {
        // Trung cấp cao: current = trung cấp cao, target = 800+
        setCurrentLevel("lr_550_650_sw_200_250")
        setTargetLevel("lr_800_sw_300")
      } else if (score >= 450) {
        // Trung cấp: current = trung cấp, target có thể là cao hoặc 800+
        setCurrentLevel("lr_450_549_sw_100_199")
        setTargetLevel("lr_550_sw_200")
      } else {
        // Cơ bản: current = cơ bản, target = trung cấp
